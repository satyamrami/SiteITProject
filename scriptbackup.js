d3.csv("SiteIT.csv").then(function (data) {
  ...
  function runEnter() {
    ...
    var inputValue = inputElement.property("value").toUpperCase();
    ...
    var filteredData = SiteIT.filter(SiteIT => SiteIT.State.toUpperCase().includes(inputValue));
    ...
    "<td>" + [i+1] + "</td>" +
    "<td>" + (output[i]['State'])+"</a>"+"</td>" + 
	"<td>" + (output[i]['Zone'])+"</a>"+"</td>" + 
    "<td>" + (output[i]['PrimaryRep'])+"</td>" +
